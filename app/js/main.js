...
console.log("\n %c KitaMusic Demo %c https://moeqy.com \n", "color: #fff; background: #ff56aa; padding:5px 0;", "background: #fbd1d7; padding:5px 0;")
var volume = 0.8;
...
var heo = {
  ...
  changeMusicBg: function (isChangeBg = true) {
    ...
    const KitaMusicBg = document.getElementById("musicBg")

    if (isChangeBg) {
      // player loadeddata 会进入此处
      const musiccover = document.querySelector("#app .aplayer-pic");
      var img = new Image();
      img.src = extractValue(musiccover.style.backgroundImage);
      img.onload = function() {
        heoMusicBg.style.backgroundImage = musiccover.style.backgroundImage;
        KitaMusicBg.style.backgroundImage = musiccover.style.backgroundImage;
      };
    } else {
      // 第一次进入，绑定事件，改背景
  ...
  addEventListenerChangeMusicBg: function () {
    const heoMusicPage = document.getElementById("app");
